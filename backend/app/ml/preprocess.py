...
def load_analyze_hdr_img_pair(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        loaded = nib.load(str(hpath), mmap=False)
        data = np.array(loaded.get_fdata(), dtype=np.float64, copy=True)
        loaded.uncache()
        del loaded
        gc.collect()
    return volume_to_axial_slice(data, slice_index)
...
def load_analyze_zip(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        loaded = nib.load(str(hdr_path), mmap=False)
        data = np.array(loaded.get_fdata(), dtype=np.float64, copy=True)
        loaded.uncache()
        del loaded
        gc.collect()
...
def analyze_hdr_img_pair_to_nifti_gz_bytes(hdr_bytes: bytes, img_bytes: bytes, stem: str = "volume") -> bytes:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ipath.write_bytes(img_bytes)
        loaded = nib.load(str(hpath), mmap=False)
        nii = nib.Nifti1Image(np.array(loaded.get_fdata()), loaded.affine, loaded.header)
